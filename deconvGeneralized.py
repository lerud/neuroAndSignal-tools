...
def deconvMain(regressor, response, eps, windowStart=0, windowEnd=None, avgRegSpec=None):
    ...
    regressorPadded=pyfftw.empty_aligned(int(regressor.shape[0]-windowStart), dtype='complex128')
    responsePadded =pyfftw.empty_aligned((int(response.shape[0]-windowStart), int(response.shape[1])), dtype='complex128')    
    fraction=pyfftw.empty_aligned((int(2*(response.shape[0]-windowStart)), int(response.shape[1])), dtype='complex128')

    regressorPadded[:]=np.pad(regressor, (0, int(-windowStart)), mode='constant', constant_values=0)
    responsePadded[:] =np.pad(response, ((int(-windowStart), 0), (0, 0)), mode='constant', constant_values=0)
    t1=time.time()
    ...
    #print(NFFT)
    calc_fft_x = fftw(regressorPadded, n=NFFT, planner_effort='FFTW_PATIENT', auto_contiguous=True)
    fft_x = calc_fft_x()
    t2=time.time()
    print(f'Finished regressor FFT; time taken was {t2-t1} seconds')
    #print(fft_x.shape)
    #print(response.shape)
    calc_fft_y = fftw(responsePadded, n=NFFT, axis=0, planner_effort='FFTW_PATIENT', auto_contiguous=True)
    fft_y = calc_fft_y()
    t3=time.time()
    ...
    print(f'Finished calculating regressor and response cross correlation; time taken was {t5-t4} seconds')
    ...
    print(f'Finished calculating regularized regressor autocovariance; time taken was {t6-t5} seconds')
    fraction[:]=numerator/denominator[:,None]
    t7=time.time()
    print(f'fraction is {fraction.shape}')
    print(f'Finished calculating least squares solution via frequency domain division; time taken was {t7-t6} seconds')

    calc_w=ifftw(fraction, n=NFFT, axis=0, planner_effort='FFTW_PATIENT', auto_contiguous=True)
    #calc_w=ifftw(numerator, n=NFFT, axis=0, planner_effort='FFTW_PATIENT', auto_contiguous=True)

    w=calc_w()
    t8=time.time()
    print(f'w is {w.shape}')
    print(f'Finished converting back to time domain via iFFT; time taken was {t8-t7} seconds')
    w=w.real
    #w=w * scale
    ...
    if windowEnd is not None:
        ...
        for i, samp in enumerate(np.linspace(int(windowStart),int(windowEnd),windowLength+1)):
            samp=int(samp)
            if samp<0:
                temp=regressor[-samp:]
                S[:samp,i]=temp
        ...
        t1=time.time()
        SScov=S.T @ S
        print(f'Regressor autocovariance matrix is shape {SScov.shape} and took {time.time()-t1} seconds to compute, max is {SScov.max()}')
        t1=time.time()
        SScov=SScov+lam*I
        print(f'Regularized autocovariance matrix is shape {SScov.shape} and took {time.time()-t1} seconds to compute')
        t1=time.time()
        SRcov=S.T @ response
        print(f'Regressor - response covariance matrix is shape {SRcov.shape} and took {time.time()-t1} seconds to compute')
        solution=np.linalg.lstsq(SScov, SRcov)
        window=SRcov
        TRF=solution[0]
        print(f'TRF matrix is shape {TRF.shape}')
        print(f'Finished least squares TRF estimation; total time taken was {time.time()-t0} seconds')
        print('\n')
