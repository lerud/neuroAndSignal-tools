...
def specAndAutocorr(inputSeries, fs, NFFT=8192, specFreqPortion=[0,100], autoFreqPortion=[0,100],
                    specWindowLength=None, windowStep=None, dynRangePortion=[0,100], autoWindowLength=None,
                    meanTimeBounds=None, tLim=None):
    ...
    y=[y[i:i+specWindowLength] for i in range(0, len(y)-specWindowLength, windowStep)]
    
    y[-1]=np.pad(y[-1], (0, len(y[-2])-len(y[-1])), mode='constant', constant_values=0)
    ...
    if meanTimeBounds is None:
        ...
        meanTimeBoundsSpec=[np.abs(meanTimeBounds[0]-tSpec).argmin(), np.abs(meanTimeBounds[1]-tSpec).argmin()]
    ...
    for count, i in enumerate(np.arange(0,len(yy)+autoWindowLength,windowStep)):
        ...
        firstNormalizedInput =(portion - np.mean(portion)) / (np.std(portion) * len(portion))
        secondNormalizedInput=(portion - np.mean(portion)) / np.std(portion)
        
        Y=np.correlate(firstNormalizedInput, secondNormalizedInput, mode='full')  # Calculate autocorrelation for each window of the zero-padded vector
    ...
    if meanTimeBounds is None:
        ...
        meanTimeBoundsAuto=[np.abs(meanTimeBounds[0]-tAuto).argmin(), np.abs(meanTimeBounds[1]-tAuto).argmin()]

    figsize=[15,12]
    #plt.figure(figsize=figsize)
    #ax1=plt.subplot(211)
    fig1, ((ax1, ax2), (ax3, ax4))=plt.subplots(nrows=2,ncols=2,figsize=figsize,width_ratios=[1,5])
    specMat=y-viewingRangeMin
    plot1=ax1.plot(-specMat[:,meanTimeBoundsSpec[0]:meanTimeBoundsSpec[1]].mean(axis=1),fPortion)
    ...
    plot3=ax3.plot(-np.nanmean(autoMat[:,meanTimeBoundsAuto[0]:meanTimeBoundsAuto[1]],axis=1),lagsAxis)
